...
all_s_files = glob.glob("./TChiWZ_BT-1_17/TChiWZ_*L/*.csv")
#print(all_s_files)
#all_s_files = glob.glob("./T2bW_BT-1_17/T2bW_*L/*.csv")

signal_set = ["TChiWZ_2500200", "TChiWZ_2500220", "TChiWZ_2500240"]
#signal_set = ["T2bW_4750465", "T2bW_5000470", "T2bW_5260475"]
...
allsframe = allsframe.loc[ ((allsframe['PType'].isin(signal_set))) ] 
#select 2L and 3L
leplist=[2,3]
allbgframe = allbgframe.loc[ ((allbgframe['NL'].isin(leplist))) ]
allsframe = allsframe.loc[ ((allsframe['NL'].isin(leplist))) ]
...
allsframe['binKey'] = allsframe['NL'].astype(int).astype(str)+"_"+allsframe['NJ'].astype(int).astype(str)+"_"+allsframe['BinN'].astype(int).astype(str)
allsframe['BinEdges'] = allsframe['binKey'].map(binmap)
...
allsframe[['Rlow','Rhigh','Mplow','Mphigh']] = pd.DataFrame(allsframe.BinEdges.tolist(), index= allsframe.index)

#print(allbgframe)
#exit()
allbgframe = allbgframe[ ['RegionName','PType','BinN','NL','NJ','Yield','Error','Rlow','Rhigh','Mplow','Mphigh'] ]
allbgframe['Rel Err'] = allbgframe.Error/allbgframe.Yield
allbgframe['Yield'] = allbgframe['Yield'].round(4)

allsframe = allsframe[ ['RegionName','PType','BinN','NL','NJ','Yield','Error','Rlow','Rhigh','Mplow','Mphigh'] ]
allsframe['Rel Err'] = allsframe.Error/allsframe.Yield
allsframe['Yield'] = allsframe['Yield'].round(4)
...
allsframe = allsframe[ ['RegionName','PType','Yield','Error','BGYield','BGError','S/B','Rlow','Rhigh','Mplow','Mphigh'] ]
#print(allsframe)
#allbgframe = allbgframe.loc[ ((allbgframe['PType'] == 'all-bkg')) ]

allsframe = allsframe.loc[ ((allsframe['PType'] == 'TChiWZ_2500200')) ]
shigh = allsframe.loc[((allsframe['S/B'] >= 0.1))]
smed = allsframe.loc[ ((allsframe['S/B'] >= 0.05) & (allsframe['S/B'] <0.1)) ]
slow = allsframe.loc[ ((allsframe['S/B'] >= 0.01) & (allsframe['S/B'] <0.05)) ]


print("S/B > 10%",shigh.shape)
print(shigh)

print("5% < S/B < 10%",smed.shape)
print(smed)

print(" 1% < S/B < 5% ",slow.shape)
print(slow)
